const rootUrlPath = process.env.VIRTUAL_PATH
  ? `${process.env.VIRTUAL_PATH}`
  : "";
  ...
  adminAuth: {
    ...
    strategy: {
      ...
      options: {
        ...
        userInfoURL: process.env.OAUTH_DISCOVERY,
        clientID: process.env.OAUTH_CLIENT_ID,
        clientSecret: process.env.OAUTH_CLIENT_SECRET,
        callbackURL: process.env.OAUTH_CALLBACK_URL,
        scope: ['email', 'profile', 'openid'],
        proxy: true,
        verify: function(issuer, profile, openid, roles, done) {
                  console.log(roles);
                  const token = JSON.stringify(roles, null, 2).split(".");
                  console.log(token);
                  const payload = JSON.parse(atob(token[1]));
		  const allowedRole = process.env.OAUTH_ALLOWED_ROLE
                  if (payload.roles.includes(allowedRole)) {
        ...
        users: function(user) {
        // Allow all authenticated users to be admins
